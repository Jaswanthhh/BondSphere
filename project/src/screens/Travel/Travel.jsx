...
export const Travel = () => {
  ...
  const handleJoinTrip = (listingId) => {
  ...
  const filteredListings = travelListings.filter(listing => {
    ...
    const matchesDateRange = (!dateRange.start || new Date(listing.startDate) >= new Date(dateRange.start)) &&
                           (!dateRange.end || new Date(listing.endDate) <= new Date(dateRange.end));
